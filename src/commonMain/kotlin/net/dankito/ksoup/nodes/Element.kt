...
import net.codinux.kotlin.concurrent.AtomicBoolean
import net.codinux.kotlin.concurrent.WeakReference
...
open class Element @JvmOverloads constructor(private var tag: Tag, baseUri: String?, internal var attributes: Attributes? = null) : Node() {
    ...
    private var childNodes: MutableList<Node>
     ...
     *
     *
     * See the query syntax documentation in [net.dankito.ksoup.select.Selector].
     *
    ...
    fun getElementsMatchingOwnText(regex: String): Elements {
        val regex = try {
            Regex(regex) // TODO: handle JS SyntaxError
        } catch (e: Throwable) {
            throw IllegalArgumentException("Regex syntax error: $regex", e)
        }
    ...
    override fun createInstanceForClone() = Element(tag, baseUri(), attributes)
