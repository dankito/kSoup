...
class HtmlTreeBuilderTest {
    ...
    fun nonnullAssertions() {
        val parseMethod = TreeBuilder::class.java.getDeclaredMethod(
            "parse",
            Reader::class.java,
            String::class.java,
            Parser::class.java
        )
        Assertions.assertNotNull(parseMethod)
        val declaredAnnotations = parseMethod.declaredAnnotations
        var seen = false
        for (annotation in declaredAnnotations) {
            if (annotation.annotationType().isAssignableFrom(ParametersAreNonnullByDefault::class.java)) seen = true
        }
