...
class ConnectIT {
    ...
    fun canInterruptBodyStringRead() {
        ...
        val runner = Thread {
            try {
                val res = Jsoup.connect(SlowRider.Companion.Url)
                    .timeout(15 * 1000)
                    .execute()
                body[0] = res.body()
            } catch (e: IOException) {
    ...
    fun totalTimeout() {
        ...
        var threw = false
        try {
            Jsoup.connect(SlowRider.Companion.Url).timeout(timeout).get()
    ...
    fun slowReadOk() {
        ...
        val doc = Jsoup.connect(SlowRider.Companion.Url)
            .data(SlowRider.Companion.MaxTimeParam, "2000") // the request completes in 2 seconds
            .get()
