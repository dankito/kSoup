...
class InterruptedServlet : BaseServlet() {
    ...
    override fun doGet(req: HttpServletRequest, res: HttpServletResponse) {
        var magnitude = req.getParameter(InterruptedServlet.Companion.Magnitude)
        magnitude = magnitude ?: ""
        res.contentType = BaseServlet.Companion.TextHtml
        res.status = HttpServletResponse.SC_OK
        val sb = StringBuilder()
        sb.append("<title>Something</title>")
        while (sb.length <= CharacterReaderTest.Companion.maxBufferLen) {
            sb.append("A suitable amount of data. \n")
        }
        sb.append("<p>Finale.</p>")
        val data = sb.toString()
        val contentLength = if (magnitude == InterruptedServlet.Companion.Larger) data.length * 2 else data.length / 2
