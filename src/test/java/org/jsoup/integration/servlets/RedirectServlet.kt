...
class RedirectServlet : BaseServlet() {
    override fun doGet(req: HttpServletRequest, res: HttpServletResponse) {
        var location = req.getParameter(RedirectServlet.Companion.LocationParam)
        if (location == null) location = ""
        var intCode: Int = RedirectServlet.Companion.DefaultCode
        val code = req.getParameter(RedirectServlet.Companion.CodeParam)
        if (code != null) intCode = code.toInt()
        if (req.getParameter(RedirectServlet.Companion.SetCookiesParam) != null) {
