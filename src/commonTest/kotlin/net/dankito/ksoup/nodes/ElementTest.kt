...
import net.codinux.kotlin.concurrent.AtomicInt
...
class ElementTest {
    ...
    fun elementsByTagName() {
        ...
        assertEquals("Hello", (ps[0].childNode(0) as TextNode).wholeText)
        assertEquals("Another ", (ps[1].childNode(0) as TextNode).wholeText)

        val ps2: List<Element> = doc.getElementsByTag("P")
        assertElementsEqual(ps, ps2)
 ...
 <div>
  5
    ...
    fun testGetDataNodes() {
        ...
        assertEquals("One Two", scriptData[0].wholeData)
        val styleData = style!!.dataNodes()
        assertEquals(1, styleData.size)
        assertEquals("Three Four", styleData[0].wholeData)
    ...
    fun insertChildrenArgumentValidation() {
        ...
        try {
           ...
           fail()
        } catch (_: NullPointerException) { } // JS throws a ClassCastExceptions, all other platforms a NullPointerException
  ...
  <div id=div-01>Here is div-01
    ...
    fun testNormalizesNbspInText() {
        ...
        assertEquals(withNbsp, p.textNodes()[0].wholeText)
    ...
    fun testNormalizesInvisiblesInText() {
        ...
        assertEquals(decoded, p.textNodes()[0].wholeText)
...
<div>
 <span>Now</span>
    ...
    fun testExpectFirstMessage() {
        ...
        try {
            ...
            threw = true
            assertEquals("No elements matched the query 'span.doesNotExist' on element 'p'.", e.message)
        }
        assertTrue(threw)
    }
    ...
    fun testExpectFirstMessageDoc() {
        ...
        try {
            ...
            threw = true
            assertEquals("No elements matched the query 'span.doesNotExist' in the document.", e.message)
        }
        assertTrue(threw)
    }
 ...
 <a>Text</a>
    ...
    fun insertChildrenValidation() {
        val doc = Jsoup.parse(reference)
        val div = doc.expectFirst("div")
        val ex: Throwable = assertFailsWith(ValidationException::class) {
    ...
    fun elementsByAttributeValueMatchingValidation() {
        val doc = Jsoup.parse(reference)
        val ex: Throwable = assertFailsWith(IllegalArgumentException::class) {
            doc.getElementsByAttributeValueMatching("key", "\\x")
        }
        assertEquals("Regex syntax error: \\x", ex.message)
    }
    ...
    fun outerHtmlAppendable() {
        ...
        // looks like a bug to me that StringBuilder should not start with '\n' but all other Appendables should
