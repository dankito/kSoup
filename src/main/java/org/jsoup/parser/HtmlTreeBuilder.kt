...
class HtmlTreeBuilder() : TreeBuilder() {
    ...
    private var formattingElements: ArrayList<Element?>? = null // active (open) formatting elements
    private var tmplInsertMode: ArrayList<HtmlTreeBuilderState>? = null // stack of Template Insertion modes
    private var pendingTableCharacters: MutableList<Token.Character?>? = null // chars in table to be shifted out
    private var emptyEnd: Token.EndTag? = null // reused empty end tag
    private var framesetOk: Boolean = false // if ok to go into frameset
    ...
    fun insert(characterToken: Token.Character?, el: Element?) {
        ...
        if (characterToken!!.isCData()) node = CDataNode(data) else if (isContentForTagData(tagName)) node = DataNode(
            (data)!!
    ...
    private fun dedupeAttributes(startTag: Token.StartTag?) {
    ...
    private fun inSpecificScope(targetName: String?, baseTypes: Array<String?>, extraTypes: Array<String?>?): Boolean {
        specificScopeTarget.get(0) = targetName
