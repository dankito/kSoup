 ...
 *
 ***Note** that using this selector will modify the DOM, so you may want to `clone` your document before using.</td><td>`p:matchText:firstChild` with input `<p>One<br />Two</p>` will return one [net.dankito.ksoup.nodes.PseudoTextElement] with text "`One`".</td></tr>
